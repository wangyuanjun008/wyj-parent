...
$(function() {
	...
	$('.switch-systems').click(function () {
		...
		$.cookie('wyj_auth_systemid', systemid);
		$.cookie('wyj_auth_systemname', systemname);
		$.cookie('wyj_auth_systemtitle', systemtitle);
	});
	// 显示cookie菜单
	var systemid = $.cookie('wyj_auth_systemid') || 1;
	var systemname = $.cookie('wyj_auth_systemname') || 'wyj_auth_server';
	var systemtitle = $.cookie('wyj_auth_systemtitle') || '权限管理系统';
	$('.system_menus').hide(0, function () {
		$('.system_' + systemid).show();
	});
	$('body').attr('id', systemname);
